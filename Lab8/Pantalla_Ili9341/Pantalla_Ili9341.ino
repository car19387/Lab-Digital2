...
String archives[10];  // String para nombre de los archivos
...
// Se definene los pines para la LCD
#define LCD_RST PD_0
#define LCD_CS PD_1
#define LCD_RS PD_2
#define LCD_WR PD_3
...
void setup() {
  ...
  // Inicialización en blanco de pantalla 
  LCD_Init();
  LCD_Clear(0x00);
  ...
  for(int i=1; i < 5;i++){      
    Serial.println(archives[i]); // Se imprimen los nombres de los archivos
    }

  // Almacenar primera imagen en File class e imprimir en pantalla
...
void loop() {
  ...
  if(digitalRead(PUSH1)== LOW && flag1 == true){ // Si se preciona y bandera está activa
    imagen ++;  // Se aumenta la variable
    flag1 = false;
    delay(100);
    if(imagen >= 2){
      imagen = 0; // Se usa anti-rebote
    }
    if(imagen == 1);{ 
      LCD_Print("IMAGEN2",0,0,2,0X0000,0XF4F4);
      }
      
    if(imagen == 0){  // Si está en 0
  ...
  if(digitalRead(PUSH2)== LOW && flag2 == true){
    if(imagen == 0){
      archivo = SD.open("Bitmap1.TXT",FILE_READ);
      bitmapSD(archivo);
      archivo.close();
...
void ListaNombres(File archivo){
  ...
  while(true){
    ...
    archives[n] = dummy.name(); // Agregar nombre a String
